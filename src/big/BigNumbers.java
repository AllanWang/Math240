...
public class BigNumbers {
    ...
    public static void printMatch(String[] full) {
        final int[] testCount = {0};
        HashMap<Long, HashSet<LinkedList<String>>> map = new HashMap<>();
        ProgressReport.start(() -> testCount[0]);
        new Combinations(full).getAllCombinations(list -> {
//            print(list.toString());
//            print("Ending %d", getEnding(list, 3));
//            print(sum(list));
            testCount[0]++;
            long ending = getEnding(list, 2);
            if (map.containsKey(ending)) {
                map.get(ending).add(list);
            } else {
                map.put(ending, new HashSet<LinkedList<String>>() {{
                    add(list);
                }});
            }
        });
        ...
        for (Long key : map.keySet()) {
            HashMap<String, LinkedList<String>> resultMap = new HashMap<>();
            HashSet<LinkedList<String>> set = map.get(key);
            for (LinkedList<String> list : set) {
                String sum = sum(list);
                if (resultMap.containsKey(sum)) {
                    print("Match found for sets\n%s\nand\n%s\nSum is %s", resultMap.get(sum), list, sum);
                    return;
                }
    ...
    private static String[] test = {
            "153", "289", "336", "456", "510", "999", "109", "789", "929", "854"
    };
